...
public class SongLoader {
    protected static final String BASE_SELECTION = AudioColumns.IS_MUSIC + "=1"
    ...
    @NonNull
    public static ArrayList<Song> getSongs(@NonNull Context context, final String query) {
        Cursor cursor = makeSongCursor(context, AudioColumns.TITLE + " LIKE ?", new String[]{"%" + query + "%"});
        return getSongs(cursor);
    }

    @NonNull
    public static ArrayList<Song> getSongsWithId(@NonNull Context context, final String[] id) {
        Cursor cursor = makeSongCursor(context, AudioColumns._ID + " IN (?)", id);
        return getSongs(cursor);
    }

    @NonNull
    public static Song getSong(@NonNull Context context, final int songId) {
        Cursor cursor = makeSongCursor(context, AudioColumns._ID + "=?", new String[]{String.valueOf(songId)});
